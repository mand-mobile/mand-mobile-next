...
export function resolveComponents({platform: PLATFORM, componentSource}): Array<IComponents> {
  ...
  return R.compose(R.filter((component) => (!component.platform || component.platform===PLATFORM)), R.map(R.compose(componentNameMapper)), R.map(loadComponent), lsComponentsDirectories)(componentSource)
}
