...
const Animate = ((global) => {
  ...
  return {
      ...
      return (callback: any) => {
        ...
        if (intervalHandle === null) {
          intervalHandle = setInterval(() => {
            const time = +new Date()
            const currentRequests: any = requests
    ...
    start(
      ...
      verifyCallback: (arg0: number) => any,
      completedCallback: (
        arg0: number,
        arg1: number,
        arg2: boolean
      ...
      const step = (virtual: boolean) => {
        ...
        if (
          !running[id] ||
          (verifyCallback && !verifyCallback(id))
        ) {
          running[id] = null
          completedCallback &&
            completedCallback(
              desiredFrames -
                dropCounter /
                  ((now - start) / millisecondsPerSecond),
              id,
              false
            )
          return
        ...
        let value = easingMethod
          ? easingMethod(percent)
          : percent
        value = isNaN(value) ? 0 : value
        if (
          (stepCallback(value, now, render) === false ||
            percent === 1) &&
          render
        ) {
          running[id] = null
